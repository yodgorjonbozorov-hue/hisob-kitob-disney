...
## 2026-09-15 — ZAKAZ TO'LOV TIZIMI: LEDGER, QOLDIQ ≠ QARZ, "YUTILDI" SHARTI

Audit natijasi: CRM zakaz to'lovida uchta mustaqil xato bir-birini
kuchaytirib turgan edi.

### Muammo qayerdan chiqqan edi

1. **Qoldiq QARZ deb o'qilardi.** `qarzUlushi()` (`lib/crm/pipeline.ts`)
   "QISMAN" holatning O'ZINI qarz deb hisoblardi. 750 000 lik zakazga
   200 000 zalog berilsa, qolgan 550 000 "Yutildi" bosilishi bilan
   mijozga QARZDORLIK bo'lib ochilardi — hech kim so'ramagan holda.
2. **"Yutildi" ning to'lov sharti YO'Q edi.** Qisman to'langan zakazni
   yutildi qilish mumkin edi (`zakazniYakunlash` faqat taqsimotni
   hisoblardi, ruxsatni emas), shu sabab 1-band avtomatik ishga tushardi.
3. **To'lov ALMASHTIRISH orqali yozilardi.** Yangi to'lov qo'shishning
   yagona yo'li butun ro'yxatni qayta yozish edi (`tolovSatrlariniYoz`
   → `deleteMany` + `create`). Forma eskirgan snapshot bilan yuborilsa
   oldingi qatorlar JIMGINA o'chardi — "50 000 naqd + 250 000 click
   kiritdim, faqat naqd qoldi" muammosining ildizi.
4. Qo'shimcha: pul faqat "Yutildi" paytida kirimga tushardi, ya'ni
   kelib tushgan zalog kassada umuman ko'rinmasdi.

### Yangi model

- **To'lovning haqiqat manbai — `DealTolov` qatorlari** (append-only
  ledger). `Deal.tolangan` — o'sha qatorlarning AYNI tranzaksiyada
  yangilanadigan keshi (doska kartochkalari uni o'qiydi, N+1 bo'lmasin).
- **Pul TO'LOV QILINGAN paytda kirimga tushadi** (`lib/crm/tolovQoshish.ts`):
  har qator o'z `Transaction` ini yozadi — naqd naqd kassaga,
  click/terminal karta/hisob kassasiga; sana — pul KELGAN kun.
- **"Yutildi" — MOLIYAVIY AMAL EMAS, holat.** U kirim yaratmaydi
  (qatorlar allaqachon bog'langan), shuning uchun dublikat mumkin emas.
- **Qoldiq ≠ Qarz.** Qarz FAQAT ikki aniq tanlovda ochiladi: zakaz
  "Qarzga" belgilangan (`tolovTuri = "qarz"`) yoki yakunlash oynasida
  `qarzgaYopish` bosilgan. Zalog berilganining o'zi hech qachon qarz emas.
- **"Yutildi" faqat to'liq to'langanda** — `yutildiTekshiruvi()` sof
  funksiyasi brauzerda ham, serverda ham ayni javobni beradi; majburlash
  `lib/crm/yakunlash.ts` da (eski "bosqichga sudrash" yo'li ham shu
  yerdan o'tadi). Ortiqcha to'lov ham jimgina o'tmaydi.

### Poyga himoyasi

- To'lov qo'shishda `Deal.tolangan` ning OLDINGI qiymati `updateMany`
  sharti (CAS) — parallel ikkinchi so'rovning butun tranzaksiyasi
  qaytariladi, dublikat to'lov ham, dublikat kirim ham tushmaydi.
- Yakunlashda holat CAS (`holat: <oldingi>`) — parallel ikki "Yutildi"
  dan faqat bittasi o'tadi.

### O'zgargan fayllar

`lib/crm/pipeline.ts` (qarzUlushi, yangi yutildiTekshiruvi),
`lib/crm/tolovQoshish.ts` (YANGI: ledger + kirim + o'chirish),
`lib/crm/yakunlash.ts`, `lib/crm/service.ts` (createDeal),
`lib/services/zakazSotuvchi.ts` (sotuvchi almashganda ZAKAZNING BARCHA
kirimlari yangilanadi — ilgari faqat birinchisi), `lib/services/audit.ts`,
`lib/validation/crm.ts`, `api/crm/deals/[id]/route.ts` (qulflar:
kategoriya kirimdan, narx yakundan; to'lov ledgerga ko'chdi),
`api/crm/deals/[id]/tolov/**` (YANGI), CRM UI: `ZakazTolovlari` (YANGI),
`ZakazSarlavha` (YANGI — 250 satr qoidasi), `BuyurtmaSheet`,
`BuyurtmaTahrir`, `YakunlashTasdiq`, `ZakazMoliya`, `TolovMaydonlari`,
`CrmClient`, `turlar.ts`.

### Tekshirish

`npm run test:zakaz-tolov` (YANGI, 10 ta: topshiriqdagi to'qqiz stsenariy
+ xato to'lovni tuzatish). Regressiya: crm-tolovlar, crm-pipeline, crm,
crm-sotuvchi, crm-xodim-kassa, zakaz-jamoasi, kpi, kpi-hisob,
xodim-statistika, xodim-plan, qarz, qarzdorlik, qarz-taqsimot,
qarz-mijoz, kunlik, kunlik-kassa, panel, dashboard-ux, agregat,
moliya-pul, moliya-audit, direktor-oqimi, pro, kirim-chiqim,
tolov-taqsimoti, isolation, izolyatsiya-royxati, atomik, soft-delete,
ai, avto, bot-avto, audit, visibility, kop-biznes, modules, backup —
hammasi yashil. `npm run build` o'tadi.
