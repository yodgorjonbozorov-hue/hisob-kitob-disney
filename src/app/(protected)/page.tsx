    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          ...
          <p className="text-2xl font-bold text-emerald-600">{formatSomLabel(summary.jamiKirim)}</p>
          <p className={`text-xs mt-1 ${changeDirection(summary.changePct.kirim) === "up" ? "text-emerald-600" : changeDirection(summary.changePct.kirim) === "down" ? "text-rose-600" : "text-slate-400"}`}>
            O'tgan oyga nisbatan {formatPercent(summary.changePct.kirim)}
          </p>
        </Card>
        <Card>
          <p className="text-slate-500 text-sm mb-1">Jami chiqim</p>
          <p className="text-2xl font-bold text-rose-600">{formatSomLabel(summary.jamiChiqim)}</p>
          <p className={`text-xs mt-1 ${changeDirection(summary.changePct.chiqim) === "up" ? "text-rose-600" : changeDirection(summary.changePct.chiqim) === "down" ? "text-emerald-600" : "text-slate-400"}`}>
        ...
        <Card className={summary.sofFoyda >= 0 ? "bg-emerald-50 border-emerald-200" : "bg-rose-50 border-rose-200"}>
          ...
          <p className={`text-2xl font-bold ${summary.sofFoyda >= 0 ? "text-emerald-600" : "text-rose-600"}`}>
            {formatSomLabel(summary.sofFoyda)}
          </p>
          <p className={`text-xs mt-1 ${changeDirection(summary.changePct.sofFoyda) === "up" ? "text-emerald-600" : changeDirection(summary.changePct.sofFoyda) === "down" ? "text-rose-600" : "text-slate-400"}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          <h2 className="font-semibold text-slate-700 mb-3">Kirim — kategoriya bo'yicha</h2>
          <CategoryPieChart data={kirimBreakdown} emptyLabel="Bu oyda kirim yo'q" />
          <CategoryRanking data={kirimBreakdown} />
        </Card>
        <Card>
          <h2 className="font-semibold text-slate-700 mb-3">Chiqim — kategoriya bo'yicha</h2>
      ...
      <Card>
        <h2 className="font-semibold text-slate-700 mb-3">So'nggi 6 oy dinamikasi</h2>
        <TrendChart data={trend} />
      </Card>

      <Card>
        <h2 className="font-semibold text-slate-700 mb-3">Kunlik dinamika (joriy oy)</h2>
      ...
      {data.map((item, i) => (
        <li key={item.nomi} className="flex items-center justify-between text-sm">
          ...
          <span className="font-medium text-slate-800">
            {formatSomLabel(item.summa)} <span className="text-slate-400">({item.foiz.toFixed(0)}%)</span>
          </span>
        </li>
      ))}
    </ul>
