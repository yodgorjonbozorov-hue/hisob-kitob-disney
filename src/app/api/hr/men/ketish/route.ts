...
import { ishniTugat } from "@/lib/services/davomat";
import { checkSchema } from "@/lib/validation/davomat";

/** CHECK-OUT — xodim ishni tugatadi (siyosatga qarab selfie/GPS qayta talab qilinadi). */
export const POST = withTenant(
  async (request, _ctx, { session: user }) => {
    ...
    return NextResponse.json(
      await ishniTugat({ businessId, employeeId: xodim.id, input: parsed.data })
    );
  },
  { module: "HR" }
);
