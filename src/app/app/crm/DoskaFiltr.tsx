...
import type { KategoriyaDTO, SotuvchiDTO, XodimDTO } from "./turlar";
...
/**
 * DOSKA FILTRI (12/25-talab): sana, SOTUVCHI, mas'ul xodim, kategoriya,
 * to'lov holati.
 *
 * "Sotuvchi" va "Mas'ul xodim" ATAYLAB ikki alohida filtr: birinchisi
 * zakazni KIM OLGANI (Employee, biriktiruv jadvali), ikkinchisi zakazni
 * kim yuritayotgani (User).
 *
...
export function DoskaFiltr({
  ...
  xodimlar: XodimDTO[];
  /** Sotuvchilar (Employee) — zakazni kim olgani bo'yicha filtr. */
  ...
  const filtrBor = Boolean(
    filtr.from || filtr.to || filtr.masulId || filtr.sotuvchiId || filtr.categoryId || filtr.tolov
  );
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
        {sotuvchilar.length > 0 && (
          <div className="space-y-1">
            ...
            <Select
              id="df-sotuvchi"
              value={filtr.sotuvchiId}
              onChange={(v) => yangila({ sotuvchiId: v || null })}
              searchable={sotuvchilar.length > 7}
              options={[
                { value: "", label: "Barchasi" },
                ...sotuvchilar.map((x) => ({ value: x.id, label: x.ism })),
              ]}
            />
          </div>
        )}
        <div className="space-y-1">
          <label className="block text-2xs text-faint" htmlFor="df-masul">Mas&apos;ul xodim</label>
          <Select
            id="df-masul"
            value={filtr.masulId}
