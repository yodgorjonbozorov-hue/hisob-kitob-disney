...
import { ZakazDirektorMoliya } from "./ZakazDirektorMoliya";
import type { BuyurtmaDTO, KategoriyaDTO, XodimDTO } from "./turlar";

const INPUT =
  "w-full rounded-lg border border-line bg-surface px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand";

/**
 * DIREKTOR TUZATISHI — zakaz nomi, mijozi, sanasi, mas'ul xodimi va HOLATI.
 *
 * ═══ NEGA ALOHIDA BLOK ═══
 ...
 *
 * ═══ NARX/TO'LOV (`moliyaTuzatadi`) ═══
 * Moliyaga o'tgan zakazning narxi va to'lovi FAQAT direktorga ochiladi va
 * alohida blokda (`ZakazDirektorMoliya`): u saqlaganda server moliyani
 * qaytarib, yangi raqamlardan qayta yozadi (`lib/crm/direktorTahrir.ts`).
 * Shu sabab bu blok "pulga tegmaydigan" maydonlardan ATAYLAB ajratilgan —
 * ikki xil xavf darajasi bitta tugmaga yig'ilmasin.
 */
export function ZakazDirektorTahriri({
  ...
  xodimlar: XodimDTO[];
  /** Kirim kategoriyalari — narx/to'lov tuzatishida kerak. */
  kategoriyalar: KategoriyaDTO[];
  /** Narx/to'lov bloki ko'rsatilsinmi (direktor + moliyaga o'tgan zakaz). */
  moliyaTuzatadi: boolean;
  /** Serverdan kelgan yangi qiymatlar (bo'lsa) — ochiq oyna yangilanadi. */
  ...
  return (
    <div className="space-y-2">
      ...
      {moliyaTuzatadi && (
        <ZakazDirektorMoliya b={b} kategoriyalar={kategoriyalar} onSaqlandi={onSaqlandi} />
      )}
    </div>
  );
}
