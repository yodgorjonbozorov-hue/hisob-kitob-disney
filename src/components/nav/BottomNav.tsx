...
/** Tab ikonkasi — yon menyu bilan bitta to'plamdan (nav/ikonlar.ts). */
function TabIcon({ name, active }: { name: string; active?: boolean }) {
  const Icon = ikon(name);
  return <Icon className="w-5 h-5" strokeWidth={active ? 2.25 : 2} aria-hidden="true" />;
}
        ...
        <div className="flex items-stretch h-14 relative">
          ...
          <div className="w-16 flex items-start justify-center">
            <button
              ...
              className="-mt-5 w-14 h-14 rounded-full bg-brand text-brand-fg shadow-raised flex items-center justify-center active:scale-95 transition"
            >
